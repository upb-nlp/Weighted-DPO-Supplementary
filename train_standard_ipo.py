"""
Standard IPO training on anonymous/Dolci-Instruct-DPO-en, starting from
allenai/Olmo-3-7B-Instruct-SFT, using the HuggingFace TRL DPOTrainer with
loss_type="ipo".

IPO (Identity Preference Optimization, arXiv:2310.12036) replaces DPO's
sigmoid log-loss with a squared loss on the log-ratio difference. In TRL,
beta acts as the IPO regularization strength tau: the per-example loss is
(logits - 1/(2*beta))**2, and the log-ratios are length-normalized
(averaged over completion tokens) rather than summed as in sigmoid DPO.
...
OUTPUT_DIR = "/checkpoints/weighted-dpo/olmo3-7b-standard-ipo-full_beta_0.1_lr_2e-6"
...
# IPO regularization strength (tau). TRL's IPO loss is length-normalized, so
# beta is interpreted differently than in sigmoid DPO; tune if needed.
BETA = 0.1
WARMUP_RATIO = 0.1
